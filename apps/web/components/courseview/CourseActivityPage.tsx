...
export default function CourseActivityPage() {
  ...
  const sections = useMemo<SectionGroup[]>(() => {
    const contents = courseDetail?.contents ?? [];
    return contents
      .map((section, index) => {
        const sectionName =
          section.name?.trim() || `Section ${section.section ?? index + 1}` ||
          "Untitled section";
        const modules =
          Array.isArray(section.modules) && section.modules.length > 0
            ? section.modules
                ...
                .map((module) => {
                  const fileUrl =
                    module?.contents?.[0]?.fileurl ?? module?.url ?? module?.modurl ??
                    null;
                  return {
                    id: String(module.id),
                    title: module.name,
                    ...
                    visible: Boolean(module.visible !== 0),
