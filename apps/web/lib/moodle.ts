...
export async function createMoodleUser(payload: {
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            ...
            throw new Error(
                `Moodle API Request Failed: HTTP ${status} - ${typeof data === "string" ? data : JSON.stringify(data)
                }`,
            );
        }
        throw error;
...
export class MoodleClient {
    ...
    async login(username: string, password: string): Promise<{ token: string }> {
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new Error(
                    `Moodle Login Failed: HTTP ${status} - ${typeof data === "string" ? data : JSON.stringify(data)
                    }`,
                );
            }
            throw error;
    ...
    async getCourses(): Promise<(MoodleCourseType & { image: string | null })[]> {
        const courses = await this.call<MoodleCourseType[]>("core_course_get_courses", {});

        const list = (Array.isArray(courses) ? courses : [])
            .filter((c: any) => c?.id !== 1 && c?.shortname !== "SITE" && c?.format !== "site");
            ...
            list.map(async (c: any) => {
                ...
                const full =
                    Array.isArray(res) ? res?.[0] :
                        Array.isArray(res?.courses) ? res.courses?.[0] :
                            null;
    ...
    async getPublicCourses(): Promise<MoodleCourseType[]> {
        ...
        const all = await Promise.all(realCats.map((c: any) => this.getCoursesByCategory(c.id)));
        return all.flat();
    }
    /**
     * Get all Categories
