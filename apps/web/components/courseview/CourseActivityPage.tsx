...
export default function CourseActivityPage() {
    ...
    const isNamedModule = (m: MoodleCourseContentModule | null | undefined): m is NamedModule =>
    ...
    const sections = useMemo<SectionGroup[]>(() => {
        ...
        return contents
            .map((section, index) => {
                const sectionName =
                    section.name?.trim() || `Section ${section.section ?? index + 1}` || "Untitled section";
                ...
                const modules =
                    Array.isArray(section.modules) && section.modules.length > 0
                        ? (section.modules as MoodleCourseContentModule[])
                            ...
                            .map((module) => {
                                const fileUrl =
                                    module?.contents?.[0]?.fileurl ?? module?.url ?? module?.modurl ?? null;

                                return {
                                    id: String(module.id),
                                    title: module.name, // now guaranteed string
                                    description: stripHtml(module.description) || FALLBACK_DESCRIPTION,
                                    section: sectionName,
    ...
    const modules = useMemo(() => sections.flatMap((section) => section.items), [sections]);
    ...
    useEffect(() => {
        ...
        if (activityIdParam && modules.some((module) => module.id === activityIdParam)) {
                ...
                <div className="space-y-6">
                    <section className="rounded-3xl border border-[var(--border)] bg-white p-6 shadow-sm">
                        ...
                        <div className="mt-2 flex items-center justify-between text-xs uppercase tracking-[0.4em] text-slate-500">
                            ...
                            <span>
                                {visitedModules.size} / {moduleCount} module{moduleCount === 1 ? "" : "s"} visited
                            </span>
                        </div>
                    </section>
        ...
        <div className="fixed inset-0 z-50">
            ...
            <div className="absolute left-0 top-0 h-full w-[360px] bg-white shadow-2xl">
                <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
                    <div className="text-sm font-semibold text-slate-900">Course menu</div>
                    <button
                        onClick={onClose}
