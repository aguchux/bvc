...
export async function GET(
  _: NextRequest,
  { params }: { params: Promise<CategoryRouteParams> },
) {
  ...
  try {
    ...
    const subcategories = Array.isArray(categoriesPayload)
      ? categoriesPayload.filter(
          (item: any) => Number(item?.parent ?? 0) === categoryId,
        )
      : [];

    const courses = Array.isArray(coursesPayload)
      ? coursesPayload.filter(
          (course: any) => Number(course?.categoryid ?? 0) === categoryId,
        )
      : [];
