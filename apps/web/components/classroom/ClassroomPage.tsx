...
const buildCourseViewModels = (
  ...
  return courses.map((course) => {
    ...
    const end =
      course.enddate && course.enddate > start
        ? course.enddate
        : start + 30 * 24 * 60 * 60;
    ...
    const status: CourseViewModel["status"] =
      nowSeconds > end ? "Past" : "In progress";
...
export default function ClassroomPage() {
  ...
  function goToCourse(course: CourseViewModel) {
    ...
    router.push(
      `/dashboard/classroom/course/${encodeURIComponent(courseId)}/activity/overview`,
    );
  }
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="mt-5 rounded-3xl border border-dashed border-slate-200 bg-white p-8 text-center text-sm text-slate-500">
          You do not have any enrolled Moodle courses yet. Once you join a
